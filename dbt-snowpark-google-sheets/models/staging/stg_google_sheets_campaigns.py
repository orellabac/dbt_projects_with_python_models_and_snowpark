...
def model(dbt, session: Session):
    ...
    secrets = {"service_account_info": "service_account_info"},
    packages=PACKAGES)

    data = json.loads(gsheets_udf(SPREADSHEET_ID, RANGE_NAME))
    return session.create_dataframe(data['data'],schema=T.StructType([
        ...
        T.StructField("spend",   T.FloatType()),      # numeric
        ...
        T.StructField("revenue", T.FloatType())]))    # numeric
